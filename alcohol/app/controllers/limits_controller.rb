class LimitsController < ApplicationController
  ...
  def index
    ...
    lis = []
    @record.each do |r|
      if r.drunk=="ふらふらする/呂律が回らない" || r.drunk=="泥酔"
        lis.push(r.toughness)
      end
    end
    #許容量の算出に使用するパラメータを準備
    ...
    if lis.length==0
      ...
      @toughness_avg = lis.sum.fdiv(lis.length).round
    end
    #レコードに"ふらふらする/呂律が回らない","泥酔"の日が含まれていない場合、または飲み会時間が0の場合は算出しない
    if drinking_hours==0 || @toughness_avg==0
      @limit_alc = '-'
    else
      @limit_alc = (limit_con+drinking_hours*dec)*@toughness_avg
    end
    ...
    @alc_hash = (start_date..end_date).map { |date| [date, 0] }.to_h
    # 飲酒記録が登録されている日付に関しては、摂取アルコール量を代入する
    @record.each do |r|
      if r.drink_day >= Date.today.beginning_of_month && r.drink_day <= Date.today.end_of_month
    ...
    lis_alc == [] ? @alc_avg = 0 : @alc_avg = lis_alc.sum.fdiv(lis_alc.length).round
    #飲み過ぎ率を算出
    overdrinking_count == 0 ? @overdrinking_ratio = 0 : @overdrinking_ratio = (overdrinking_count.to_f / @record.length * 100).round
  end
end
