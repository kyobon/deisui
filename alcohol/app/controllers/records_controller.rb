...
class RecordsController < ApplicationController
  before_action :logged_in_user, only: %i[new create destroy_record edit_by_day update_record]
  ...
  def destroy_record
    ...
    @user = User.find(current_user.id)
    @record = @user.records.find_by(drink_day:)
  ...
  def edit_by_day
    ...
    @record = @user.records.find_by(drink_day:) # 該当する日付のレコードを取得
  end
  ...
  def update_record
    ...
    if @record.present? # レコードが存在するか確認
      if @record.update(record_params) # レコードの更新
        ...
        toughness = if ['ふらふらする/呂律が回らない', '泥酔'].include?(drunk)
                      calculate_toughness(@record) # calculate_toughnessメソッドで酒の強さを算出
                    end
        @record.toughness = toughness
        if @record.save
  ...
  def record_params
    params.require(:record).permit(:beer, :highball, :chuhi, :sake, :wine, :whiskey, :shochu, :drink_day, :hours,
                                   :minutes, :drunk)
  end
end
